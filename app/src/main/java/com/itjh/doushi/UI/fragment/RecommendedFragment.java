...
public class RecommendedFragment extends BaseFragment implements OnMoreListener, BaseQuickAdapter.OnItemClickListener<VideoEntity> {
    ...
    private BaseQuickAdapter<VideoEntity, BaseAdapterHelper> mAdapter;
...
//                simpleDraweeView.setLayoutParams(layoutParams);
    ...
    public void onMoreAsked(int overallItemsCount, int itemsBeforeMore, int maxLastVisiblePosition) {
        retrofit.create(VideoService.class).listRepos((mAdapter.getItem(mAdapter.getItemCount() - 1)).id, "20", videoType, "0").subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(videoListResponse -> {
            Logger.i(videoListResponse.toString());
            mAdapter.appendToList(videoListResponse.content);
        }, throwable -> {
